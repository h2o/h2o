...
<dd>Size of the QPACK encoder table, in bytes. Controls how much of the dynamic table h2o will use when encoding response headers. Default is 4,096.</dd>
<dt>qpack-decoder-table-capacity</dt>
<dd>Size of the QPACK decoder table, in bytes. Setting this to a non-zero value advertises <code>SETTINGS_QPACK_MAX_TABLE_CAPACITY</code> to the peer, allowing it to use dynamic-table references when encoding request headers. h2o will park request streams that depend on not-yet-received encoder-stream inserts until the references are resolved, bounded by the negotiated blocked-streams budget. Default is 0 (disabled).</dd>
