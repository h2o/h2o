...
enum h2olog_event_id_t {
  ...
  H2OLOG_EVENT_ID_H2O_ASYNC_IO_START_SPLICE_FILE,
  H2OLOG_EVENT_ID_H2O_ASYNC_IO_END,
...
struct h2olog_event_t {
  ...
  union {
    ...
    struct { // h2o:async_io_start_splice_file
      struct st_h2o_socket_read_file_cmd_t * cmd;
    } async_io_start_splice_file;
    struct { // h2o:async_io_end
      struct st_h2o_socket_read_file_cmd_t * cmd;
    } async_io_end;
...
void h2o_raw_tracer::initialize() {
  available_usdts.assign({
    ...
    h2o_tracer::usdt("h2o", "async_io_start_splice_file", "trace_h2o__async_io_start_splice_file"),
    h2o_tracer::usdt("h2o", "async_io_end", "trace_h2o__async_io_end"),
...
void h2o_raw_tracer::do_handle_event(const void *data, int data_len) {
  ...
  case H2OLOG_EVENT_ID_H2O_ASYNC_IO_START_SPLICE_FILE: { // h2o:async_io_start_splice_file
    json_write_pair_n(out_, STR_LIT("type"), STR_LIT("async-io-start-splice-file"));
    json_write_pair_c(out_, STR_LIT("tid"), event.tid);
    json_write_pair_c(out_, STR_LIT("seq"), seq_);
    json_write_pair_c(out_, STR_LIT("cmd"), event.async_io_start_splice_file.cmd);
    json_write_pair_c(out_, STR_LIT("time"), time_milliseconds());
    break;
  }
  case H2OLOG_EVENT_ID_H2O_ASYNC_IO_END: { // h2o:async_io_end
    json_write_pair_n(out_, STR_LIT("type"), STR_LIT("async-io-end"));
    json_write_pair_c(out_, STR_LIT("tid"), event.tid);
    json_write_pair_c(out_, STR_LIT("seq"), seq_);
    json_write_pair_c(out_, STR_LIT("cmd"), event.async_io_end.cmd);
    json_write_pair_c(out_, STR_LIT("time"), time_milliseconds());
    break;
  }
...
// h2o:async_io_start_splice_file
int trace_h2o__async_io_start_splice_file(struct pt_regs *ctx) {
  const void *buf = NULL;
  struct h2olog_event_t event = { .id = H2OLOG_EVENT_ID_H2O_ASYNC_IO_START_SPLICE_FILE, .tid = (uint32_t)bpf_get_current_pid_tgid(), };

  { // struct st_h2o_socket_read_file_cmd_t * cmd
    bpf_usdt_readarg(1, ctx, &event.async_io_start_splice_file.cmd);
  }

  if (events.perf_submit(ctx, &event, sizeof(event)) != 0)
    bpf_trace_printk("failed to perf_submit in trace_h2o__async_io_start_splice_file\n");

  return 0;
}
// h2o:async_io_end
int trace_h2o__async_io_end(struct pt_regs *ctx) {
  const void *buf = NULL;
  struct h2olog_event_t event = { .id = H2OLOG_EVENT_ID_H2O_ASYNC_IO_END, .tid = (uint32_t)bpf_get_current_pid_tgid(), };

  { // struct st_h2o_socket_read_file_cmd_t * cmd
    bpf_usdt_readarg(1, ctx, &event.async_io_end.cmd);
  }

  if (events.perf_submit(ctx, &event, sizeof(event)) != 0)
    bpf_trace_printk("failed to perf_submit in trace_h2o__async_io_end\n");
