...
my $doit = sub {
    my ($proto, $opt, $port) = @_;
    my $out = `nghttp $opt -nv -d $huge_file -m 10 $proto://127.0.0.1:$port/echo | grep 'recv WINDOW_UPDATE' | grep -v stream_id=0 | grep -oP stream_id=.. | uniq -c | wc -l`;
    chomp($out);
    is $out, 10, "No interleaving";
};
