...
static int check_weight_distribution(h2o_balancer_backend_t *backends, size_t backends_len)
{
    size_t i, j;

    for (i = 0; i < backends_len; i++) {
        for (j = i + 1; j < backends_len; j++) {
            if (backends[i].conn_count * ((unsigned)backends[j].weight_m1 + 1) !=
                backends[j].conn_count * ((unsigned)backends[i].weight_m1 + 1))
                return 0;
        }
    }
    return 1;
...
static void test_round_robin(void)
{
    ...
    for (i = 0; i < 10; i++)
        total_count += ((unsigned)real_backends[i].weight_m1) + 1;
    total_count *= 1000;
