  ...
  default:
    paths:
      /:
        file.dir: @{[ DOC_ROOT ]}
EOT
    test(spawn_h2o({conf => $conf, max_ssl_version => 'TLSv1.2', quic => { disable => 1 }}), "-sess_out $tempdir/session", "New");
    test(spawn_h2o({conf => $conf, max_ssl_version => 'TLSv1.2', quic => { disable => 1 }}), "-sess_in $tempdir/session", ["New", "Reused"]); # At the first request, redis connection hasn't been established
};
...
EOT
    test(spawn_h2o({conf => $conf, max_ssl_version => 'TLSv1.2', quic => { disable => 1 }}), "-sess_out $tempdir/session", "New");
    test(spawn_h2o({conf => $conf, max_ssl_version => 'TLSv1.2', quic => { disable => 1 }}), "-sess_in $tempdir/session", ["New", "New"]);
};
