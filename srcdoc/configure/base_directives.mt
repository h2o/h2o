...
<p>
If the value is <code>all</code>, H2O always sends informational responses to the client if the protocol defines 1xx status codes (e.g. HTTP/1.0 doesn't define that).
</p>
<p>
If the value is <code>none</code>, H2O never sends informational responses to the client.
</p>
<p>
If the value is <code>except-h1</code>, H2O sends informational if the protocol is not HTTP/1.*.
</p>
