...
static void request_next_cb(h2o_timeout_entry_t *entry)
{
...
void h2o_request_next(h2o_req_t *req)
{
...
void h2o_loop_context_init(h2o_loop_context_t *ctx, uv_loop_t *loop)
{
    ...
    ctx->loop = loop;
    h2o_timeout_init(&ctx->request_next_timeout, 0, request_next_cb, loop);
