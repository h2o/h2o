...
<h4>PROXY attribute</h4>
<p>
This boolean attribute (i.e. the value of the attribute must be either <code>ON</code> or <code>OFF</code>) specifies if the server should recognize the information passed via <a href="http://www.haproxy.org/download/1.5/doc/proxy-protocol.txt">"the PROXY protocol</a> in the incoming connections.
...
EOT
?>
<h4>Defining the Methods Used</h4>
<p>
The <code>method</code> attribute defines which methods should be used for resuming the TLS sessions.
...
<h4>Attributes for Cache-based Resumption</h4>
<p>
...
<h4>Attributes for Ticket-based Resumption</h4>
<p>
...
<h4>The Lifetime Attribute</h4>
<p>
The <code>lifetime</code> attribute defines the lifetime of a TLS session; when it expires the session cache entry is purged, and establishing a new connection will require a full TLS handshake.
Default value is <code>3600</code> (in seconds).
</p>
<h4>The Memcached Attribute</h4>
<p>
The <code>memcached</code> attribute specifies the location of memcached used by the <code>memcached</code> stores.  The value must be a mapping with <code>host</code> attribute specifying the address of the memcached server, and optionally a <code>port</code> attribute specifying the port number (default is <code>11211</code>).
</p>
? })
