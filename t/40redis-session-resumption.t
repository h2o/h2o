  ...
  default:
    paths:
      /:
        file.dir: @{[ DOC_ROOT ]}
EOT
    test(spawn_h2o({conf => $conf, max_ssl_version => 'TLSv1.2'}, {quic => 0}), "-sess_out $tempdir/session", "New");
    test(spawn_h2o({conf => $conf, max_ssl_version => 'TLSv1.2'}, {quic => 0}), "-sess_in $tempdir/session", ["New", "Reused"]); # At the first request, redis connection hasn't been established
};
...
EOT
    test(spawn_h2o({conf => $conf, max_ssl_version => 'TLSv1.2'}, {quic => 0}), "-sess_out $tempdir/session", "New");
    test(spawn_h2o({conf => $conf, max_ssl_version => 'TLSv1.2'}, {quic => 0}), "-sess_in $tempdir/session", ["New", "New"]);
};
...
subtest 'load test' => sub {
    ...
    my $redis = spawn_redis();
    my $server = spawn_h2o({conf => << "EOT", max_ssl_version => 'TLSv1.2'}, {quic => 0});
ssl-session-resumption:
  lifetime: 3
  mode: cache
  cache-store: redis
